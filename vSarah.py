...
class NetworkServerController:
	...
	self.s = s
		...
		(lsock,_,_) = select.select(liste_socket+[s],[],[],0)   #le 0 correspond au delais d'attente avant que ça bloque
		for sock in lsock:      #les sockets parmi la liste
		    if sock == s and lsock:
		        (joueur,ip) = s.accept() #on accepte un nouveau client aka nouvelle socket
		        print("test 1")
		        msg = joueur.recv(1000)
		        liste_socket.append(joueur)
		        print("debug : le client est connecté")
		        model = msg.encode('utf_8')
		        print("debug :",model)
		        sock.send(model)
		    else :
